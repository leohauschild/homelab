resource "proxmox_vm_qemu" "workers" {
  ...
  name        = format("%s-%s", local.workers.name_prefix, count.index)
  boot        = local.boot
  agent       = local.agent
  ...
  ipconfig0 = format("ip=%s/24,gw=%s", cidrhost(local.cidr, local.workers.network_last_octect + count.index), cidrhost(local.cidr, 1))
